...
@section('content')
    <div class="card">
        ...
        <div class="p-6">
            <form id="edit_invoice_form" action="{{ route('invoices.update', ['invoice' => $invoice->encrypted_id()]) }}"
                method="POST">
                @csrf
                @method('PUT')

                <div class="grid grid-cols-1 md:grid-cols-2  gap-6">
                    <x-input type="date" label="Invoice Date" name="invoice_date"
                        value="{{ old('invoice_date') ?? $invoice->invoice_date->format('Y-m-d') }}" required />

                    <x-input type="date" label="Due Date" name="due_date"
                        value="{{ old('due_date') ?? $invoice->due_date->format('Y-m-d') }}" required />

                    <x-input type="number" label="PO Number" name="po_number"
                        value="{{ old('po_number') ?? $invoice->po_number }}" placeholder="Enter PO Number" required />

                    <x-select label="Timezone" name="timezone_id" :options="$timezones"
                        selected="{{ old('timezone_id') ?? $invoice->timezone_id }}" required />

                    <x-select label="Sale Owner" name="owner_id" :options="$staffs" placeholder="Select Sale Owner"
                        selected="{{ old('owner_id') ?? $invoice->owner_id }}" />

                    <x-select label="Organization" name="organization_id" :options="$organizations"
                        placeholder="Select Organization"
                        selected="{{ old('organization_id') ?? $invoice->organization_id }}" />
