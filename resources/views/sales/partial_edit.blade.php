<div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 mb-4">
    <table class="responsive table">
        ...
        <tr>
            <td>{{$solution->name}}</td>
            <td><input type="number" class="form-control" name="quantity[]" id="quantity_{{$solution->id}}" onkeyup="getAmount({{$solution->id}}); getTotalPrice(); getFinalPrice()" @if(isset($saleSolutions[$key]->quantity)) value="{{$saleSolutions[$key]->quantity}}" @else value="1" @endif></td>
            <td><input type="number" class="form-control" name="unit_price" id="price_{{$solution->id}}" value="{{$solution->price}}" readonly></td>
            <td><input type="number" name="discount_percentage[]" id="discount_percentage_{{$solution->id}}" class="form-control" @if(isset($saleSolutions[$key]->discount_percentage)) value="{{$saleSolutions[$key]->discount_percentage}}" @else value="0.00" @endif onkeyup="getAmount({{$solution->id}}); getTotalPrice(); getFinalPrice()"></td>
...
<script>
    $(document).ready(function(){
        ...
        $('.amount').each(function(){
            let amountRowId= $(this).attr('id');
            let id= amountRowId.replace('amount_', '');
            getAmount(id);
